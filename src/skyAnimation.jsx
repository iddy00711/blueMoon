import React, {useMemo, useState, useEffect} from 'react';
import {
  TouchableWithoutFeedback,
  Image,
  StyleSheet,
  Dimensions,
  View,
  Text
} from 'react-native';
import Animated, { Easing, stopClock } from 'react-native-reanimated';
import HomePage from './HomePage'

const imageSize = {
  width: 256,
  height: 256,
};

const screenWidth = Dimensions.get('window').width;
const animatedWidth = screenWidth + imageSize.width;

const {
  useCode,
  block,
  set,
  Value,
  Clock,
  eq,
  clockRunning,
  not,
  cond,
  startClock,
  timing,
  interpolate,
  and,
} = Animated;

const runTiming = (clock) => {
  const state = {
    finished: new Value(0),
    position: new Value(0),
    time: new Value(0),
    frameTime: new Value(0),
  };

  const config = {
    duration: 5000,
    toValue: 1,
    easing: Easing.inOut(Easing.linear),
  };

  return block([
    // we run the step here that is going to update position
    cond(
      not(clockRunning(clock)),
      set(state.time, 0),
      timing(clock, state, config),
    ),
    cond(eq(state.finished, 1), [
      set(state.finished, 0),
      set(state.position, 0),
      set(state.frameTime, 0),
      set(state.time, 0),
    ]),
    state.position,
  ]);
}

export default function SkyAnimation () {
  const [play, setPlay] = useState(true);
  const {progress, clock, isPlaying} = useMemo(
    () => ({
      progress: new Value(0),
      isPlaying: new Value(0),
      clock: new Clock(),
    }),
    [],
  );

  useEffect(() => {
    isPlaying.setValue(play ? 1 : 0);
  }, [play, isPlaying]);

  useCode(
    () =>
      block([
        cond(and(not(clockRunning(clock)), eq(isPlaying, 1)), startClock(clock)),
        cond(and(clockRunning(clock), eq(isPlaying, 0)), stopClock(clock)),
        set(progress, runTiming(clock)),
      ]),
    [progress, clock],
  );

  const translateX = interpolate(progress, {
    inputRange: [0, 1],
    outputRange: [0, -imageSize.width],
  });

  return (
    <HomePage>

      <View style={styles.container}>
     
      <View style={{backgroundColor:'white', justifyContent:'flex-end'}}>
          <Text style={{fontSize:20, color:'black',height:'50%', width:'50%', position: 'relative', zIndex:1}}>
              Hello
          </Text>
      </View>
      
          
    <TouchableWithoutFeedback
      style={styles.touchable}
      onPress={() => setPlay(!play)}
    >
      <Animated.View style={[styles.image, { transform: [{ translateX }]}]}>
        {/* <Image
          style={styles.image}
          source={require('./clouds3.jpg')}
          resizeMode="repeat"
        /> */}
      </Animated.View>
    </TouchableWithoutFeedback>
    

      </View>
    </HomePage>
  
  );
};

const styles = StyleSheet.create({
  container: {
    flex: 1,
    justifyContent:'flex-end'
    
    
  },
  touchable:{flex:0.5, height:'50%', },

  image: {
    width: animatedWidth,
    height: '100%',
  },
  button:{
      flex:0.3,
      color:'white',
      position: 'relative',
      margin: 'auto',
      
    
   
    zIndex: -1,
    position:'relative',
  }
});




// import React, {useMemo, useState, useEffect} from 'react';
// import {Image, StyleSheet, TouchableWithoutFeedback, Dimensions} from 'react-native';
// import Animated, { stopClock, Easing } from 'react-native-reanimated'


// const imageDimensions = {
//     width:230,
//     height:230
// };
// const screenWidth = Dimensions.get('window').width;

// let imageWidth = imageDimensions.width+screenWidth;

// const {useCode, block , set, Clock, Value, eq, not, clockRunning, cond, timing, interpolate, and} = Animated;

// function runTiming(clock) {
//     const state = {
//       finished: new Value(0),
//       position: new Value(0),
//       time: new Value(0),
//       frameTime: new Value(0),
//     };
  
//     const config = {
//       duration: 5000,
//       toValue: 1,
//       easing: Easing.inOut(Easing.linear),
//     };
  
//     return block([
     
//       // we run the step here that is going to update position
//       timing(clock, state, config),
     
//       // we made the block return the updated position
//       state.position,
//     ]);
//   }

// export default function SkyAnimation(){
//     const {progress, clock} = useMemo(()=>({
//         progress:new Value(0),
//         clock: new Clock,
//     }), []);
//     useCode(()=>{
//         block([
//             cond(not(clockRunning(clock))),startClock(clock), set(progress, runTiming(clock))
//         ]), [progress, clock]
//     })
//     const translateX = interpolate(progress, {
//         inputRange:[0,1],
//         outputRange:[0, -imageWidth]
//     })
//     // if(!clockRunning(clock)){
//     //     startClock(clock)
//     // }
//     return(
//         <TouchableWithoutFeedback style={styles.body, {transform:[{translateX}]}} >
//         <Animated.View style={styles.image}>

//             <Image style={styles.image} source={require('../image/clouds.jpg')} resizeMode='repeat'/>
//         </Animated.View>
//         </TouchableWithoutFeedback>
//     )
// }


// const styles = StyleSheet.create({
//     body:{
//         flex:1,
//     },
//     image:{
//         width:'100%',
//         height:'100%'
//     }
// })